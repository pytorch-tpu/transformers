...
def get_sampler(dataset, args):
    if args.local_rank == -1 and not args.use_tpu:
...
def train(args, train_dataset, model, tokenizer):
    ...
    train_sampler = get_sampler(train_dataset, args)
    if args.use_tpu:
    ...
    loss = None
    model.zero_grad()
    train_iterator = trange(int(args.num_train_epochs), desc="Epoch", disable=args.local_rank not in [-1, 0] or args.use_tpu)
    set_seed(args)  # Added here for reproductibility (even between python 2 and 3)
    for _ in train_iterator:
        epoch_iterator = tqdm(train_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0] or args.use_tpu)
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    model_to_save.save_pretrained(output_dir, xla_device=args.use_tpu)
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        # Also note that we don't shard for TPU Multiprocess as we don't reduce loss among client processes.
        ...
        logger.info("  Num examples = %d", num_batches * args.eval_batch_size)
...
def main(args):
    ...
    # Setup for using TPUs
    if args.use_tpu:
        if not _TORCH_XLA_INSTALLED:
            raise ImportError('Could not import torch_xla package. Please install torch_xla first:'
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model_to_save = model.module if hasattr(model, 'module') else model  # Take care of distributed/parallel training
        model_to_save.save_pretrained(args.output_dir, xla_device=args.use_tpu)
...
def _tpu_mp_fn(rank, args):
    global logger
    logger_name ='{} [{}]'.format(__name__, xm.get_ordinal(defval=-1))
...
def main_cli():
    args = get_args()
    if args.use_tpu:
        xmp.spawn(_tpu_mp_fn, args=(args,), nprocs=args.num_cores)
    else:
        main(args)
